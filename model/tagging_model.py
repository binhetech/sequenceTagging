...
class TaggingModel(BaseModel):
    """Specialized class of Model for Sequence Tagging"""

    def __init__(self, config):
        super(TaggingModel, self).__init__(config)
    ...
    def add_placeholders(self):
        ...
        # tokenLevel = 2
        # shape = (batch size, max length of sentence, max length of word)
        self.sentence_ids = tf.placeholder(tf.int32, shape=[None, None, None], name="sentence_ids")

        # shape = (batch_size, max_length of sentence)
        self.sentence_lengths = tf.placeholder(tf.int32, shape=[None, None], name="sentence_lengths")
    ...
    def get_feed_dict(self, samples, labels=None, lr=None, dropout=None):
        """Given some data, pad it and build a feed dictionary

        Args:
            samples: list of sample. A sample is a list of token.
            labels: list of ids
        ...
        if self.config.tokenLevel == 0:
            # char embedding
            char_ids, word_ids = zip(*samples)
            word_ids, sequence_lengths = pad_sequences(word_ids, 0)
            char_ids, word_lengths = pad_sequences(char_ids, pad_tok=0, nlevels=0)
        ...
        if self.config.tokenLevel == 0:
            ...
            feed[self.sentence_ids] = sentence_ids
    ...
    def add_word_embeddings_op(self):
        ...
        with tf.variable_scope("sentences"):
            if self.config.tokenLevel == 2:
                ...
                # bi lstm on sentence
                cell_fw = tf.contrib.rnn.LSTMCell(self.config.hidden_size_sentence,
                                                  state_is_tuple=True)
    ...
    def run_epoch(self, train, dev, epoch):
        ...
        for i, (samples, labels) in enumerate(minibatches(train, batch_size)):
            fd, _ = self.get_feed_dict(samples, labels, self.config.lr, self.config.dropout)
    ...
    def predict(self, words_raw):
        ...
        preds = [self.idx2tag[idx] for idx in list(pred_ids[0])]
